"""
Adaptive Reward V2 Manager for verl GRPO training.

Extends V1 with intra-group diversity bonus:
- Extracts UIDs from DataProto to identify GRPO groups (N responses per prompt)
- Passes UIDs and training step to the reward function
- Diversity bonus encourages minority format types within each group
- Diversity weight decays over training steps (exploration -> exploitation)
"""
...
@register("adaptive_reward_v2")
class AdaptiveRewardV2Manager(AbstractRewardManager):
    """
    Reward manager for AdaptiveRewardV2 with diversity bonus.

    Decodes responses with skip_special_tokens=False to preserve
    <perception>, <reasoning>, <answer> tags for format detection.
    Extracts UIDs for intra-group diversity computation.
    """
    ...
    def __init__(
        ...
        try:
            ...
            self.reward_fn = AdaptiveRewardV2(
                ...
                initial_diversity_weight=initial_diversity_weight,
                diversity_decay_strategy=diversity_decay_strategy,
                total_training_steps=total_training_steps,
                diversity_exp_decay_rate=diversity_exp_decay_rate,
            )
            print(
                f"Loaded AdaptiveRewardV2 (skip_special_tokens=False, "
                f"diversity_weight={initial_diversity_weight}, "
                f"decay={diversity_decay_strategy}, "
                f"total_steps={total_training_steps})"
            )
        except ImportError as e:
            raise RuntimeError(
                f"Failed to import AdaptiveRewardV2 from reward_functions/adaptive_reward_v2.py. "
                f"Error: {e}"
            )
    ...
    def __call__(self, data: DataProto, return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        """
        Compute rewards for a batch of responses with diversity bonus.

        Extracts UIDs from DataProto to group responses for diversity computation.
        """
        # Extract training step and total steps from metadata
        if hasattr(data, 'meta_info'):
            if 'training_step' in data.meta_info:
                step = data.meta_info['training_step']
                self.set_training_step(step)
            if 'total_training_steps' in data.meta_info:
                self.reward_fn.set_total_training_steps(data.meta_info['total_training_steps'])
        ...
        for i in range(len(data)):
            ...
            # Extract UID for group diversity computation
            uid = data_item.non_tensor_batch.get("uid", f"__default_{i}")
